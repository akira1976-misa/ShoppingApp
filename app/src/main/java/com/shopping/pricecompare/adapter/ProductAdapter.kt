...
class ProductAdapter(
    ...
    inner class VH(v: View) : RecyclerView.ViewHolder(v) {
        ...
        fun bind(p: Product) {
            ...
            if (p.isFreeShipping) {
                ship.text = "+ 무료배송"
                ship.setTextColor(itemView.context.getColor(R.color.free_shipping_color))
            } else {
                ship.text = "+ 배송비 ${fmt.format(p.lowestShipping)}원"
                ship.setTextColor(itemView.context.getColor(R.color.text_secondary))
            }

            total.text  = "최저 ${fmt.format(p.lowestTotalPrice)}원"
            stars.rating = p.rating
            rat.text    = p.rating.toString()
            rev.text    = "(${fmt.format(p.reviewCount)})"
            card.setOnClickListener { onProductClick(p) }
        }
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
        val layout = if (isHorizontal) R.layout.item_product_horizontal
