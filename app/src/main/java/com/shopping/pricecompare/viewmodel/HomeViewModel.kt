...
class HomeViewModel : ViewModel() {
    data class CategorySection(val category: String, val title: String, val products: List<Product>)
    ...
    fun loadSections() {
        viewModelScope.launch {
            ...
            val result = SampleData.categories.drop(1).map { cat ->
                ...
                CategorySection(cat, if (items.any { it.isSpecialDeal }) "🔥 $cat 핫딜" else "💰 $cat 최저가", items)
