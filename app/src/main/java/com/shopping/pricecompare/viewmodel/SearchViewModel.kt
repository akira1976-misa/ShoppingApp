...
class SearchViewModel : ViewModel() {
    ...
    fun changeSort(sort: SortOption) {
        currentSort = sort
        applyAndUpdate()
    }
            ...
            val products = if (result.items.isNotEmpty()) {
                ShoppingRepository.convertItems(result.items).let { list ->
                    if (category.isNotBlank() && category != "전체")
                        list.map { it.copy(category = category) }
                    else list
                }
            } else {
    ...
    private fun applyAndUpdate() {
        ...
        if (f.selectedBrands.isNotEmpty()) {
            list = list.filter { p ->
                f.selectedBrands.any { b -> p.name.contains(b, ignoreCase = true) }
            }
        }
    ...
    private fun extractBrands(products: List<Product>) {
        val known = listOf("삼성","애플","LG","소니","나이키","아디다스",
            "뉴발란스","유니클로","노스페이스","다이슨","쿠쿠","테팔",
            "설화수","라네즈","닥터자르트","가민","필립스","보스")
        _availableBrands.value = known.filter { brand ->
            products.any { it.name.contains(brand, ignoreCase = true) }
        }
    }
}
