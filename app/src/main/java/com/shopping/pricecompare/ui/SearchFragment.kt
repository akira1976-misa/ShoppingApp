...
class SearchFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // 검색창 클릭 → 상품 목록으로 (검색 모드)
        b.searchBarMain.setOnClickListener {
            findNavController().navigate(
                R.id.action_search_to_productList,
                bundleOf("searchMode" to true, "category" to "전체")
            )
        }

        // 카테고리 칩은 카테고리 탭으로 이동 유도
        b.categoryGrid.removeAllViews()
        val hint = android.widget.TextView(requireContext()).apply {
            text = "카테고리 탐색은 하단 [카테고리] 탭을 이용해 주세요 →"
            textSize = 13f
            setPadding(0, 24, 0, 0)
            setTextColor(requireContext().getColor(com.shopping.pricecompare.R.color.text_secondary))
        }
